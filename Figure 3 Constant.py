...
def NetAGTP_Constant(t, alpha, decay, AANonCO2,tauNonCO2):
    return AGTPNonCO2_Final_partial(t, AANonCO2=AANonCO2, tauNonCO2=tauNonCO2) + alpha * AGTPPRF_Constant_partial(t=t, decay=decay)

# tCDR vs CO2, F(t, alpha, decay)
def NetAGTP_Constant_CO2(t, alpha, decay):
    return AGTPCO2_partial(t) + alpha * AGTPPRF_Constant_partial(t=t, decay=decay)
...
if True:
    ...
    def find_TpeakCO2(decay):
        ...
        """
        def objective(t):
            return AGTPPRF_Constant_partial(t=t, decay=decay)
    ...
    # save 
    np.savetxt('data/taus_Constant.csv', taus, delimiter=',')
    np.savetxt('data/TpeakCO2_values_Constant.csv', TpeakCO2_values, delimiter=',')
    np.savetxt('data/tauNonCO2_values_Constant.csv', tauNonCO2_values, delimiter=',')

# read TpeakCO2 and tauNonCO2 values from CSV files
taus = np.loadtxt('data/taus_Constant.csv', delimiter=',')
TpeakCO2_values = np.loadtxt('data/TpeakCO2_values_Constant.csv', delimiter=',')
tauNonCO2_values = np.loadtxt('data/tauNonCO2_values_Constant.csv', delimiter=',')
...
for decay, color in zip(decays, tCDR_colors):
    ax.plot(t, 1E15*AGTPPRF_Constant_partial(t=t, decay=decay), '--',
...
ax.text(0.6, 7.9, "{:.1f} years for 100-year storage".format(y_ref), size=14)   
...
ax.text(0.6, 21, "{:.1f} years for 100-year storage".format(y_ref), size=14)  
...
plt.savefig('figure/Figure 3 Constant.png', bbox_inches='tight', pad_inches=0.1, dpi=300)
plt.savefig('figure/Figure 3 Constant.pdf', bbox_inches='tight', pad_inches=0.1)