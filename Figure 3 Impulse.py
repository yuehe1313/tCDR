...
def NetAGTP_Impulse(t, alpha, decay, AANonCO2,tauNonCO2):
    return AGTPNonCO2_Final_partial(t, AANonCO2=AANonCO2, tauNonCO2=tauNonCO2) + alpha * AGTPPRF_Impulse_partial(t=t, decay=decay)

# tCDR vs CO2, F(t, alpha, decay)
def NetAGTP_Impulse_CO2(t, alpha, decay):
    return AGTPCO2_partial(t) + alpha * AGTPPRF_Impulse_partial(t=t, decay=decay)
...
if True:
    ...
    #     """
    #     def objective(t):
    #         return AGTPPRF_Impulse_partial(t=t, decay=decay)
    ...
    # save 
    np.savetxt('data/taus_Impulse.csv', taus, delimiter=',')
    np.savetxt('data/TpeakCO2_values_Impulse.csv', TpeakCO2_values, delimiter=',')
    np.savetxt('data/tauNonCO2_values_Impulse.csv', tauNonCO2_values, delimiter=',')

# read TpeakCO2 and tauNonCO2 values from CSV files
taus = np.loadtxt('data/taus_Impulse.csv', delimiter=',')
TpeakCO2_values = np.loadtxt('data/TpeakCO2_values_Impulse.csv', delimiter=',')
tauNonCO2_values = np.loadtxt('data/tauNonCO2_values_Impulse.csv', delimiter=',')
...
for decay, color in zip(decays, tCDR_colors):
    ax.plot(t, 1E15*AGTPPRF_Impulse_partial(t=t, decay=decay), '--',
...
ax.text(0.6, 9.2, "9.0 years for 100-year and near infinite storage", size=14)  
...
ax.text(0.6, 31.9, "30.9 years for 100-year and near infinite storage", size=14)  
...
plt.savefig('figure/Figure 3 Impulse.png', bbox_inches='tight', pad_inches=0.1, dpi=300)
plt.savefig('figure/Figure 3 Impulse.pdf', bbox_inches='tight', pad_inches=0.1)