...
def NetAGTP_Linear(t, alpha, decay, AANonCO2,tauNonCO2):
    return AGTPNonCO2_Final_partial(t, AANonCO2=AANonCO2, tauNonCO2=tauNonCO2) + alpha * AGTPPRF_Linear_partial(t=t, decay=decay)

# tCDR vs CO2, F(t, alpha, decay)
def NetAGTP_Linear_CO2(t, alpha, decay):
    return AGTPCO2_partial(t) + alpha * AGTPPRF_Linear_partial(t=t, decay=decay)
...
if True:
    ...
    def find_TpeakCO2(decay):
        ...
        """
        def objective(t):
            return AGTPPRF_Linear_partial(t=t, decay=decay)
    ...
    # save 
    np.savetxt('data/taus_Linear.csv', taus, delimiter=',')
    np.savetxt('data/TpeakCO2_values_Linear.csv', TpeakCO2_values, delimiter=',')
    np.savetxt('data/tauNonCO2_values_Linear.csv', tauNonCO2_values, delimiter=',')

# read TpeakCO2 and tauNonCO2 values from CSV files
taus = np.loadtxt('data/taus_Linear.csv', delimiter=',')
TpeakCO2_values = np.loadtxt('data/TpeakCO2_values_Linear.csv', delimiter=',')
tauNonCO2_values = np.loadtxt('data/tauNonCO2_values_Linear.csv', delimiter=',')

# Modify 
TpeakCO2_values[650:] = 9
tauNonCO2_values[650:] = 30.9

# Smooth values from index 630 to the end
TpeakCO2_values[630:] = np.linspace(TpeakCO2_values[630], 9, num=len(TpeakCO2_values[630:]))
tauNonCO2_values[630:] = np.linspace(tauNonCO2_values[630], 30.9, num=len(tauNonCO2_values[630:]))
...
for decay, color in zip(decays, tCDR_colors):
    ax.plot(t, 1E15*AGTPPRF_Linear_partial(t=t, decay=decay), '--',
...
ax.text(0.6, 7.0, "{:.1f} years for 100-year storage".format(y_ref), size=14)   
...
ax.text(0.6, 15.4, "{:.1f} years for 100-year storage".format(y_ref), size=14)  
...
plt.savefig('figure/Figure 3 Linear.png', bbox_inches='tight', pad_inches=0.1, dpi=300)
plt.savefig('figure/Figure 3 Linear.pdf', bbox_inches='tight', pad_inches=0.1)