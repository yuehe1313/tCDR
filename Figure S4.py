...
def C_Trian(t, p, rotation):
        ...
        result = np.where(condition1, term1,
                        np.where(condition2, term2,
                                np.where(condition3, term3, term4)))
        return result
...
# tCDR
ax.plot(np.arange(0, rotation + dt, dt) + xlag, 10 * F_Trian_Exp(np.arange(0, rotation + dt, dt), decay=decayValue, p = p, rotation = rotation), color=color3,linestyle='-', linewidth=2)
ax.plot(np.arange(rotation+dt, tmax + dt, dt) + xlag, 10 * F_Trian_Exp(np.arange(rotation+dt, tmax + dt, dt), decay=decayValue, p = p, rotation = rotation), color=color3, linewidth=2)
...
# pCDR-b
ax.plot(np.arange(0, rotation + dt, dt) + xlag, 10 * F_Trian_Exp(np.arange(0, rotation + dt, dt), decay=decayValue, p = p, rotation = rotation), color='k',linestyle=':', linewidth=2)

ax.plot([tmax + xlag, tmax + xlag],[-1,1], linestyle='--', color='gray', linewidth=2)
ax.plot([rotation + xlag, rotation + xlag],[-1,1], linestyle='--', color='gray', linewidth=2)
# ax.plot([xlag, xlag],[-1,1], linestyle='--', color='gray', linewidth=2)
...
ax.text(0.05, 1.02, 'Emissions (C flux)', transform=ax.transAxes,size=Fontsize)
ax.text(0.95, 0.42, 'Year', transform=ax.transAxes,size=Fontsize)  
ax.text(0.06, 0.54, 't = 0', transform=ax.transAxes,  size=Fontsize, color='k')  

ax.text(0.6, 0.85, 'tCDR only (release)', color=color3, 
        transform=ax.transAxes, size=Fontsize, 
        horizontalalignment='center', verticalalignment='top')

ax.text(0.17, 0.85, 'tCDR or pCDR \n(capture)', 
        transform=ax.transAxes, size=Fontsize, 
        horizontalalignment='center', verticalalignment='top')


arrow = FancyArrowPatch((xlag, -0.8), (rotation+xlag, -0.8),
                        arrowstyle='<|-|>', 
                        mutation_scale=20, 
                        linestyle='--',
                        color='black')
...
arrow = FancyArrowPatch((xlag+rotation, -0.8), (tmax+xlag, -0.8),
                        arrowstyle='<|-|>', 
                        mutation_scale=20, 
                        linestyle='--',
                        color='black')
...
ax.text(0.64, 0.24, r'Storage timescale ($\tau$)', 
        transform=ax.transAxes, size=Fontsize, 
        horizontalalignment='center', verticalalignment='center')

ax.text(0.18, 0.24, r'Rotation (r)', 
        transform=ax.transAxes, size=Fontsize, 
        horizontalalignment='center', verticalalignment='center')
...
# pCDR-a
ax.plot([xlag, xlag], [0, 0.8], color='k', linewidth=2)
ax.plot([xlag, tmax + xlag], [0.8, 0.8], 'k', linewidth=2)

tmp = -0.08 * np.cumsum(F_Trian_Exp(tt, decay=decayValue, p = p, rotation = rotation))

rotation_index = np.searchsorted(tt, rotation)

rotation_index = np.searchsorted(tt, rotation)

tmp1 = tmp[:rotation_index]
tmp2 = tmp[rotation_index:]
time_array = tt + xlag
time_array1 = time_array[:rotation_index]
time_array2 = time_array[rotation_index:]

# tCDR
ax.plot(time_array1, tmp1, linestyle='-', color=color3, linewidth=2)
ax.plot(time_array2, tmp2, color=color3, linewidth=2)

# pCDR-b
ax.plot(time_array1, tmp1, linestyle=':', color='k', linewidth=2)
...
ax.text(0.05, 1.02, 'Biomass (C stock)', transform=ax.transAxes,size=Fontsize)
ax.text(0.95, 0.42, 'Year', transform=ax.transAxes,size=Fontsize)  
ax.text(0.06, 0.44, 't = 0', transform=ax.transAxes,  size=Fontsize, color='k')  
...
F_Trian_Exp_result = F_Trian_Exp(tt, decay=decayValue, p=p, rotation=rotation)
tCDR = 1E2 * scalefactor * np.convolve(AGTPCO2_partial(t=tt), F_Trian_Exp_result)[:len(tt)] * dt

C_Trian_result = C_Trian(tt, p=p, rotation=rotation)
pCDRb = 1E2 * scalefactor * np.convolve(AGTPCO2_partial(t=tt), C_Trian_result)[:len(tt)] * dt
pCDRa = -1E2 * scalefactor * AGTPCO2_partial(t=tt)

# tCDR
ax.plot(tt + xlag, tCDR,linewidth=2, color=color3)
# pCDR-b
ax.plot(tt + xlag, pCDRb,linewidth=2, color='k', linestyle=':')
# pCDR-a
ax.plot(tt + xlag, pCDRa, linewidth=2, color='k')

ax.set_xlim([0,tlimit])
ax.set_ylim([-0.7,0.7])
ax.plot([rotation + xlag, rotation + xlag],[-0.55,0.55], linestyle='--', color='gray', linewidth=2)
ax.plot([xlag, xlag],[-0.55,0.55], linestyle='--', color='gray', linewidth=2)
ax.plot([tmax+xlag, tmax+xlag],[-0.55,0.55], linestyle='--', color='gray', linewidth=2)
...
ax.text(0.95, 0.42, 'Year', transform=ax.transAxes,size=Fontsize)  
ax.text(0.45, 0.3, r'tCDR (r = 50 years, $\tau$ = 100 years)', transform=ax.transAxes,size=Fontsize, color=color3)  
ax.text(0.09, 0.068, r'pCDR-a ($\tau \; \to \; \infty$)', transform=ax.transAxes,size=Fontsize, color='k')  
ax.text(0.38, 0.12, r'pCDR-b (r = 50 years, $\tau \; \to \; \infty$)', transform=ax.transAxes,size=Fontsize, color='k')  
...
plt.savefig('figure/Figure S4.png', bbox_inches='tight', pad_inches=0.1, dpi=300)
plt.savefig('figure/Figure S4.pdf', bbox_inches='tight', pad_inches=0.1)
